...
public class AutoScalingService {
    ...
    public void setReplicaCount(String serviceName, int count) {
        ...
        int prev = getReplicaCount(serviceName);
        replicaCounts.computeIfAbsent(serviceName, k -> new AtomicInteger(1)).set(clamped);
        addEvent(ScalingEvent.manual(serviceName, prev, clamped));
        log.info("MANUAL SCALE: {} set from {} to {} replicas", serviceName, prev, clamped);
    }
